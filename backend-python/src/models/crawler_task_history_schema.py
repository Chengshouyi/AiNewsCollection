"""本模組定義爬蟲任務歷史記錄的 Schema 類別，包括創建、更新和讀取的資料驗證與轉換功能。"""
...
TaskId = Annotated[
    int,
    BeforeValidator(
        validate_positive_int("task_id", is_zero_allowed=False, required=True)
    ),
]
ArticlesCount = Annotated[
    Optional[int],
    BeforeValidator(
        validate_positive_int("articles_count", is_zero_allowed=True, required=False)
    ),
]
Message = Annotated[
    Optional[str],
    BeforeValidator(validate_str("message", max_length=65536, required=False)),
]
TaskStatusValidator = Annotated[
    TaskStatus, BeforeValidator(validate_task_status("task_status", required=True))
]
StartTime = Annotated[
    Optional[datetime], BeforeValidator(validate_datetime("start_time", required=True))
]
EndTime = Annotated[
    Optional[datetime], BeforeValidator(validate_datetime("end_time", required=False))
]
