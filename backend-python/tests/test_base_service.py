"""測試 BaseService 及其相關元件的功能。

此模組包含對 BaseService、測試用的 Repository 和 Service 子類別的測試案例，
旨在驗證基礎服務層的 CRUD 操作、事務管理、錯誤處理和儲存庫互動。
"""
...
from sqlalchemy import (
    create_engine,
    String,
)  # Keep create_engine for initialized_db_manager? No, db_manager_for_test handles it. Remove.
from sqlalchemy.orm import (
    sessionmaker,
    Mapped,
    mapped_column,
    Session,
)  # Remove Session if not directly used. Keep Mapped, mapped_column for ModelForTest. Remove sessionmaker
...
from src.database.database_manager import (
    DatabaseManager,
)  # Keep for type hinting? Maybe not needed if service takes manager. Remove.
...
class RepositoryForTest(BaseRepository[ModelForTest]):
    ...
    def get_schema_class(
        self, schema_type: SchemaType = SchemaType.CREATE
    ) -> Type[Union[ModelCreateSchemaForTest, ModelUpdateSchemaForTest]]:
        """根據操作類型返回對應的schema類"""
    ...
    def validate_data(
        self, entity_data: Dict[str, Any], schema_type: SchemaType
    ) -> Dict[str, Any]:
    ...
    def create(self, entity_data: Dict[str, Any]) -> Optional[ModelForTest]:
        ...
        try:
            ...
            ):  # Should not happen if validation fails, but check anyway
    ...
    def update(
        ...
        try:
            ...
            if not validated_payload:
                logger.debug(
                    f"更新 ModelForTest (ID={entity_id}) 驗證後的 payload 為空，無需更新資料庫。"
                )
            ...
            logger.error(
                f"更新 ModelForTest (ID={entity_id}) 時發生未預期錯誤: {e}",
                exc_info=True,
            )
            raise DatabaseOperationError(
                f"更新 ModelForTest (ID={entity_id}) 時發生未預期錯誤: {e}"
...
class ServiceForTest(BaseService[ModelForTest]):
    def _get_repository_mapping(self) -> Dict[str, tuple]:
        return {"test_repo": (RepositoryForTest, ModelForTest)}
    ...
    def create_test_entity(self, data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            with self._transaction() as session:
                repo = cast(
                    RepositoryForTest, self._get_repository("test_repo", session)
                )
                ...
                if new_entity:
                    ...
                    if entity_id is None:
                        ...
                        logger.error(
                            f"創建實體後 Flush，但 ID 仍然為 None: {new_entity}"
                        )
                        return {
                            "success": False,
                            "message": "創建後無法獲取實體 ID",
                            "entity_id": None,
                        }
        ...
        try:
            with self._transaction() as session:
                ...
                if updated_entity:
                    ...
                    existing_check = repo.get_by_id(
                        entity_id
                    )  # Check again within same transaction
                    if existing_check:
                        return {
                            "success": True,
                            "message": "實體更新完成 (無變更)",
                        ...
                        return {
                            "success": False,
                            "message": f"ID={entity_id} 不存在，無法更新",
                            "updated_data": None,
                        }
    ...
    def delete_test_entity(self, entity_id: int) -> Dict[str, Any]:
        ...
        ) as e:  # Catch potential IntegrityValidationError from repo.delete
...
class TestBaseService:
    ...
    def initialized_db_manager(self, db_manager_for_test):
        ...
        try:
            # Ensure ModelForTest's table is created
            db_manager_for_test.create_tables(Base)  # Use Base from the imported model
            yield db_manager_for_test
        finally:
            logger.debug("Dropping tables after test function...")
            # Correct way to drop tables using SQLAlchemy metadata and the engine
            try:
                Base.metadata.drop_all(db_manager_for_test.engine)
                logger.debug("Tables dropped successfully.")
            except Exception as e:
                logger.error(f"Error dropping tables: {e}", exc_info=True)

    @pytest.fixture(scope="function")
    def test_service(self, initialized_db_manager):
        """為每個測試建立 TestService 實例，使用 initialized_db_manager"""
        # Service takes the DatabaseManager instance directly
        service = ServiceForTest(initialized_db_manager)
        # No explicit cleanup call needed here unless ServiceForTest has specific cleanup logic
        # The db_manager handles its own session/connection lifecycle
        yield service
        # If service had resources (e.g., caches), cleanup here: service.cleanup()

    @pytest.fixture(scope="function")
    def sample_entity_data(self):
        """建立測試資料"""
        return {"name": "test_name", "title": "測試標題"}
    ...
    def test_create_entity_validation_error(self, test_service):
        ...
        assert (
            "Validation Error" in result["message"] or "驗證失敗" in result["message"]
        )
    ...
    def test_transaction_rollback(self, test_service, sample_entity_data):
        ...
        try:
            with test_service._transaction() as session:  # pylint: disable=W0212
                repo = cast(
                    RepositoryForTest,
                    test_service._get_repository(
                        "test_repo", session
                    ),  # pylint: disable=W0212
                )
                new_entity = repo.create(
                    sample_entity_data
                )  # Use repo create directly to get ID within tx
                if new_entity:
