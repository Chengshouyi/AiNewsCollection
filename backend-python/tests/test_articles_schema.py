"""本模組測試 ArticleCreateSchema 與 ArticleUpdateSchema 的資料驗證功能。"""
...
class TestArticleCreateSchema:
    ...
    def test_missing_required_fields(self):
        ...
        for i, test_case in enumerate(test_cases):
            ...
            assert (
                "以下必填欄位缺失或值為空/空白:" in error_message
            ), f"測試案例 {i+1}: 錯誤訊息應提示欄位缺失或為空"
            assert (
                missing_field in error_message
            ), f"測試案例 {i+1}: 錯誤訊息應包含遺失的欄位 {missing_field}"
    ...
    def test_article_title_empty_validation(self):
        ...
        assert "以下必填欄位缺失或值為空/空白: title" in str(exc_info.value)
    ...
    def test_article_link_empty_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ArticleCreateSchema.model_validate(data)
        assert "以下必填欄位缺失或值為空/空白: link" in str(exc_info.value)
    ...
    def test_article_source_empty_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ArticleCreateSchema.model_validate(data)
        assert "以下必填欄位缺失或值為空/空白: source" in str(exc_info.value)
...
class TestArticleUpdateSchema:
    ...
    def test_update_created_at_not_allowed(self):
        ...
        data = {"title": "更新的文章標題", "created_at": datetime.now(timezone.utc)}
        with pytest.raises(ValidationError) as exc_info:
            ArticleUpdateSchema.model_validate(data)
    ...
    def test_update_task_id_with_invalid_value(self):
        """測試更新 task_id 欄位的無效值"""
        data = {"task_id": "not_an_integer"}  # 無效的整數值
        with pytest.raises(Exception):  # Pydantic 會自動驗證型別
            ArticleUpdateSchema.model_validate(data)
    ...
    def test_get_updated_fields(self):
        ...
        for field in required_fields:
            ...
            ), f"欄位 {field} 應該存在於 updated_fields 列表中"
    ...
    def test_update_immutable_fields_not_allowed(self):
        ...
        for field_data in immutable_fields_test_cases:
            ...
            data = {"title": "更新的文章標題", **field_data}
            with pytest.raises(ValidationError) as exc_info:
                ArticleUpdateSchema.model_validate(data)
            assert f"不允許更新 {field_name} 欄位" in str(
                exc_info.value
            ), f"更新 {field_name} 應該引發錯誤"
