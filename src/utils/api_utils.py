...
def parse_and_validate_common_query_params(args: MultiDict) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    # --- 處理分頁參數 (page, per_page, limit, offset) ---
    ...
    try:
        ...
        validated_params['limit'] = limit # Store None if not provided
        ...
        validated_params['offset'] = offset # Store None if not provided

    except ValueError as e:
        logger.warning(f"分頁參數轉換錯誤: page='{page_str}', per_page='{per_page_str}', limit='{limit_str}', offset='{offset_str}' - {e}")
        # 重新拋出 ValueError，讓 API 路由處理
        raise ValueError(f"請求參數錯誤: {e}") from e

    # --- 處理排序參數 ---
    validated_params['sort_by'] = args.get('sort_by') # 不需要嚴格驗證，由 Repository 處理
    ...
    # --- 處理通用搜尋關鍵字 'q' ---
    validated_params['q'] = args.get('q') # q 的驗證留給具體的 API 端點

    # --- 收集剩餘參數作為過濾條件 ---
    for key, value in args.items():
        if key not in STANDARD_KEYS:
            # 注意: request.args 對於同名參數可能有多個值，這裡只取第一個
            # 如果需要處理多值情況，需要使用 args.getlist(key)
