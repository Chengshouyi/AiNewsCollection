...
class BaseRepository(Generic[T], ABC):
    ...
    def validate_data(
        ...
        logger.debug(
            f"使用 Schema '{schema_class.__name__}' 驗證資料 ({schema_type.name})"
        )
        try:
            # 執行核心 Pydantic 驗證
    ...
    def _apply_filters(self, query, filter_criteria: Dict[str, Any]):
        ...
        for field, value in filter_criteria.items():
            if not hasattr(self.model_class, field):
                logger.warning(
                    f"過濾條件中的欄位 '{field}' 在模型 {self.model_class.__name__} 中不存在，將忽略此條件。"
                )
                continue
                ...
                for operator, operand in value.items():
                    if operator == "$in":
                        if isinstance(operand, list):
                            ...
                            logger.warning(
                                f"欄位 '{field}' 的 $in 操作符需要一個列表作為操作數，收到 {type(operand)}，忽略。"
                            )
                    elif operator == "$nin":
                        if isinstance(operand, list):
                            ...
                            logger.warning(
                                f"欄位 '{field}' 的 $nin 操作符需要一個列表作為操作數，收到 {type(operand)}，忽略。"
                            )
                    elif operator == "$ne":
                        ...
                        logger.warning(
                            f"欄位 '{field}' 的操作符 '{operator}' 不支援，忽略。"
                        )
            else:
                # 默認相等比較
    ...
    def find_paginated(
        ...
        if local_is_preview and preview_fields:
            ...
            if valid_preview_fields:
                ...
                logger.warning(
                    f"預覽模式請求的欄位 {preview_fields} 均無效，將返回完整物件。"
                )
        ...
        total = self.execute_query(
            lambda: count_query.count(), err_msg="計算總記錄數時出錯"
        )
