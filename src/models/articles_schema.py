"""本模組定義了文章相關的 Pydantic schema，包含創建、更新和讀取文章的資料結構與驗證邏輯。"""
...
# 通用字段定義
Title = Annotated[
    str, BeforeValidator(validate_str("title", max_length=500, required=True))
]
Link = Annotated[
    str, BeforeValidator(validate_url("link", max_length=1000, required=True))
]
Source = Annotated[
    str, BeforeValidator(validate_str("source", max_length=50, required=True))
]
SourceUrl = Annotated[
    str, BeforeValidator(validate_url("source_url", max_length=1000, required=True))
]
PublishedAt = Annotated[
    Optional[datetime],
    BeforeValidator(validate_datetime("published_at", required=False)),
]
Summary = Annotated[
    Optional[str], BeforeValidator(validate_str("summary", 10000, required=False))
]
Content = Annotated[
    Optional[str], BeforeValidator(validate_str("content", 65536, required=False))
]
Category = Annotated[
    Optional[str], BeforeValidator(validate_str("category", 100, required=False))
]
Author = Annotated[
    Optional[str], BeforeValidator(validate_str("author", 100, required=False))
]
ArticleType = Annotated[
    Optional[str], BeforeValidator(validate_str("article_type", 20, required=False))
]
Tags = Annotated[
    Optional[str], BeforeValidator(validate_str("tags", 500, required=False))
]
IsAiRelated = Annotated[
    bool, BeforeValidator(validate_boolean("is_ai_related", required=True))
]
IsScraped = Annotated[
    bool, BeforeValidator(validate_boolean("is_scraped", required=True))
]
ScrapeStatus = Annotated[
    Optional[ArticleScrapeStatus],
    BeforeValidator(validate_article_scrape_status("scrape_status", required=True)),
]
ScrapeError = Annotated[
    Optional[str], BeforeValidator(validate_str("scrape_error", 1000, required=False))
]
LastScrapeAttempt = Annotated[
    Optional[datetime],
    BeforeValidator(validate_datetime("last_scrape_attempt", required=False)),
]
TaskId = Annotated[
    Optional[int], BeforeValidator(validate_int("task_id", required=False))
]
...
class PaginatedArticleResponse(BaseModel):
    ...
    items: Union[List[ArticleReadSchema], List[Dict[str, Any]]]
    page: int
    per_page: int
    total: int
    total_pages: int
