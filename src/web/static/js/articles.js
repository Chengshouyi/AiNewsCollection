...
function renderArticlesTable(articles) {
    ...
    if (articles.length === 0) {
        tableBody.append('<tr><td colspan="7" class="text-center">暫無文章</td></tr>');
        return;
    }

    articles.forEach(article => {
        // 格式化日期
        const publishDate = article.published_date ? new Date(article.published_date).toLocaleString() : '-';
        const crawlDate = article.crawled_date ? new Date(article.crawled_date).toLocaleString() : '-';
                    ...
                    <div class="article-header mb-4">
                        <div class="d-flex justify-content-between mb-3">
                            <div>
                                ...
                                <span class="badge bg-secondary me-2">發布日期: ${article.published_date ? new Date(article.published_date).toLocaleString() : '-'}</span>
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                ...
                $('#article-publish-date').text(article.published_date ? new Date(article.published_date).toLocaleString() : '-');
                ...
                relatedArticles.forEach(article => {
                    html += `
                        <li class="list-group-item">
                            ...
                            <small class="d-block text-muted">${article.published_date ? new Date(article.published_date).toLocaleString() : '-'}</small>
                        </li>
                    `;
                });
