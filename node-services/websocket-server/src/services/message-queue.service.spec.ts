...
describe('MessageQueueService', () => {
  ...
  describe('訊息佇列管理', () => {
    ...
    it('應該正確將訊息加入佇列', () => {
      service.queueMessage('test-room', mockMessage);
      const messages = service.getQueuedMessages('test-room');
    ...
    it('當獲取佇列訊息後應該清空佇列', () => {
      service.queueMessage('test-room', mockMessage);
      service.getQueuedMessages('test-room');
      const messages = service.getQueuedMessages('test-room');
      expect(messages).toHaveLength(0);
    });

    it('當獲取不存在的房間訊息時應該返回空陣列', () => {
      const messages = service.getQueuedMessages('non-existent-room');
      expect(messages).toHaveLength(0);
    });
  });
  ...
  describe('訊息歷史記錄管理', () => {
    ...
    it('應該正確儲存訊息到歷史記錄', () => {
      service.saveToHistory('test-room', mockMessages[0]);
      const history = service.getMessageHistory('test-room');
    ...
    it('應該限制歷史記錄大小', () => {
      for (const message of mockMessages) {
        service.saveToHistory('test-room', message);
      }
      const history = service.getMessageHistory('test-room');
    ...
    it('應該正確獲取指定數量的歷史訊息', () => {
      ...
      const history = service.getMessageHistory('test-room', 5);
      expect(history).toHaveLength(5);
    });

    it('當獲取不存在的房間歷史記錄時應該返回空陣列', () => {
      const history = service.getMessageHistory('non-existent-room');
      expect(history).toHaveLength(0);
    });

    it('應該正確清除歷史記錄', () => {
      service.saveToHistory('test-room', mockMessages[0]);
      service.clearHistory('test-room');
      const history = service.getMessageHistory('test-room');
      expect(history).toHaveLength(0);
    });
  });
