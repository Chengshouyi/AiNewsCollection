...
import { BaseMessage, SystemMessage, AckMessage } from './interfaces/message.interface';
import { WsException } from '@nestjs/websockets';
import * as uuid from 'uuid'; // Add this line to import the entire module
...
describe('AppService', () => {
  ...
  describe('setSocketServer', () => {
    ...
    it('should register error and connection handlers on the server', () => {
      ...
      expect(mockIoServer.on).toHaveBeenCalledWith('error', expect.any(Function));
      ...
      if (serverErrorHandler) {
          ...
          expect(mockLoggerService.error).toHaveBeenCalledWith('Socket.IO server error:', testError, AppService.name);
      }
    });
  });
   ...
   describe('handleConnection', () => {
    ...
    beforeEach(() => {
       ...
       delete mockSocket['errorListener'];
       delete mockSocket['disconnectListener'];
       delete mockSocket['pingListener'];
     ...
     it('should handle ping event', () => {
        connectionHandler(socketInstance);
        const pingListener = mockSocket['pingListener'];
        expect(pingListener).toBeDefined();
        pingListener(); // Simulate ping event
        expect(mockSocket.emit).toHaveBeenCalledWith('pong', { timestamp: expect.any(Date) });
      });
     ...
     it('should handle socket errors by calling handleClientError', () => {
       ...
       const errorListener = mockSocket['errorListener'];
       expect(errorListener).toBeDefined();

       // Spy on the private method handleClientError to ensure it's called
       const handleClientErrorSpy = jest.spyOn(service as any, 'handleClientError');
     ...
     it('should handle disconnect with "transport close" reason by calling handleReconnection', () => {
        connectionHandler(socketInstance);
        const disconnectListener = mockSocket['disconnectListener'];
        expect(disconnectListener).toBeDefined();

        const handleReconnectionSpy = jest.spyOn(service as any, 'handleReconnection');
        disconnectListener('transport close'); // Simulate disconnect

        expect(mockLoggerService.log).toHaveBeenCalledWith(`Client ${mockSocket.id} disconnected: transport close`, AppService.name);
      ...
      it('should handle disconnect with other reasons without attempting reconnection', () => {
        ...
        expect(mockLoggerService.log).toHaveBeenCalledWith(`Client ${mockSocket.id} disconnected: io server disconnect`, AppService.name);
  ...
  describe('handleReconnection', () => {
    ...
    it('should schedule a reconnection attempt, increment count, and log success if reconnected', () => {
        ...
        expect(mockLoggerService.log).toHaveBeenCalledWith(`Client ${socketInstance.id} reconnected successfully on attempt 1`, AppService.name);
        expect((service as any).reconnectAttempts.has(socketInstance.id)).toBe(false);
    });
    ...
    it('should log warning and remove client if max attempts exceeded', () => {
       ...
       expect(mockLoggerService.warn).toHaveBeenCalledWith(`Client ${socketInstance.id} already exceeded max reconnection attempts before scheduling attempt ${maxAttempts + 1}`, AppService.name);
       expect((service as any).reconnectAttempts.has(socketInstance.id)).toBe(false);
    });
     ...
     it('should schedule next attempt with increased delay if not connected after first timeout', () => {
        ...
        expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`Scheduling reconnection attempt 1 for client ${socketInstance.id}`), AppService.name);
        ...
        expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`Scheduling reconnection attempt 2 for client ${socketInstance.id}`), AppService.name);


        // Fast-forward second timeout (2000ms)
        jest.advanceTimersByTime(2000);
        // Log for client still not connected after attempt 2
        expect(mockLoggerService.log).toHaveBeenCalledWith(`Client ${socketInstance.id} still not connected after attempt 2.`, AppService.name);
        ...
        expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`Scheduling reconnection attempt 3 for client ${socketInstance.id}`), AppService.name);
      });
  });
  ...
  describe('sendSystemMessage', () => {
    it('should emit a system_message event with the correct structure', () => {
      ...
      const expectedMessage: SystemMessage = {
       ...
       expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`sendSystemMessage:`), AppService.name); // Basic log check
      ...
      expect(socketInstance.emit).toHaveBeenCalledWith('system_message', expectedMessage);
  ...
  describe('emitToRoom', () => {
   ...
   describe('emitToAll', () => {
     it('should call io.emit with the correct arguments', () => {
       ...
       expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`emitToAll: ${event}`), AppService.name);
       expect(mockIoServer.emit).toHaveBeenCalledWith(event, data);
   ...
   describe('emitToRoomExcludingSender', () => {
     it('should call io.to(room).except(senderId).emit', () => {
       ...
       expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`emitToRoomExcludingSender: ${room} ${event}`), AppService.name);
       expect(ioServer.to).toHaveBeenCalledWith(room);
   ...
   describe('emitToRoomIncludingSender', () => {
     it('should call io.to(room).emit', () => {
       ...
       expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`emitToRoomIncludingSender: ${room} ${event}`), AppService.name);
       expect(ioServer.to).toHaveBeenCalledWith(room);
  ...
  describe('sendMessageToRoom', () => {
     it('should call emitToRoom with "new_message" event and payload', () => {
       ...
       expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`sendMessageToRoom: ${room} ${message} ${senderId}`), AppService.name);
  ...
  describe('updateTaskProgress', () => {
     it('should call emitToRoom with "task_progress" event and payload for the task room', () => {
       ...
       expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`updateTaskProgress: ${taskId} ${status} ${progress}`), AppService.name);
       expect(emitToRoomSpy).toHaveBeenCalledWith(
         taskRoom,
         'task_progress',
         expect.objectContaining({
   ...
   describe('sendMessageWithAck', () => {
    ...
    afterEach(() => {
     ...
     it('should resolve true when ACK is received successfully', async () => {
       ...
       const ackMessage: AckMessage = { messageId: message.id, status: 'received', timestamp: new Date() };
       ...
       expect(mockLoggerService.log).toHaveBeenCalledWith(`Message ${message.id} acknowledged`, AppService.name);
     ...
     it('should resolve false when ACK indicates failure', async () => {
       ...
       const ackMessage: AckMessage = { messageId: message.id, status: 'failed', error: 'Processing error', timestamp: new Date() };
       ...
       expect(mockLoggerService.error).toHaveBeenCalledWith(`Message ${message.id} failed: Processing error`, AppService.name);
     ...
     it('should resolve false on timeout', async () => {
       ...
       await expect(promise).resolves.toBe(false);
       expect(mockLoggerService.warn).toHaveBeenCalledWith(`Message ${message.id} ACK timeout`, AppService.name);
    ...
    describe('handleMessageAck (Private Method)', () => {
      it('should emit "message_ack" event with correct payload', () => {
       ...
       it('should include error in payload when status is "failed"', () => {
          ...
          expect(mockLoggerService.log).toHaveBeenCalledWith(`handleMessageAck: ${messageId} ${status} ${error}`, AppService.name);
          expect(socketInstance.emit).toHaveBeenCalledWith('message_ack', expectedAck);
      });
    });
   ...
   describe('broadcastMessage', () => {
     ...
     const message: BaseMessage = { id: 'bcast1', type: 'system', timestamp: new Date(), sender: 'systemSender' };
     ...
     it('should use emitToRoomIncludingSender when includeSender is true', () => {
       ...
       expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`broadcastMessage: ${room}`), AppService.name);
     ...
     it('should use emitToRoomExcludingSender when includeSender is false', () => {
       ...
       expect(mockLoggerService.log).toHaveBeenCalledWith(expect.stringContaining(`broadcastMessage: ${room}`), AppService.name);
       expect(emitExcludingSpy).toHaveBeenCalledWith(room, 'message', message, message.sender);
