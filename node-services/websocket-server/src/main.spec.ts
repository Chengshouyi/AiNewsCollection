...
const mockSocketIoServerConstructor = jest
  .fn()
  .mockReturnValue(mockIoServerInstance);
jest.mock('socket.io', () => ({
  Server: jest
    .fn()
    .mockImplementation((...args: any[]) =>
      mockSocketIoServerConstructor(...args),
    ),
}));
...
describe('bootstrap', () => {
  ...
  it('should initialize NestJS app, HTTP and Socket.IO servers with default port and CORS', async () => {
    ...
    expect(mockCreateServer).toHaveBeenCalledWith(
      mockHttpAdapterInstance.getInstance(),
    );
    ...
    expect(mockLoggerLog).toHaveBeenCalledWith(
      'WebSocket Server is running on port: 15001',
      'Bootstrap',
    );
  });
  ...
  it('should use CORS_ORIGIN from config if available', async () => {
    ...
    await bootstrap();
    expect(mockSocketIoServerConstructor).toHaveBeenCalledWith(
      mockHttpServer,
  ...
  describe('Socket.IO connection events', () => {
    ...
    it('should handle "join_room" event', () => {
      ...
      expect(mockLoggerLog).toHaveBeenCalledWith(
        `Client ${mockClientSocket.id} joined room: room123`,
        'SocketRoom',
      );
    });
    ...
    it('should handle "error" event', () => {
      ...
      expect(mockLoggerError).toHaveBeenCalledWith(
        `Socket error for client ${mockClientSocket.id}`,
        testError,
        'SocketError',
      );
    });
