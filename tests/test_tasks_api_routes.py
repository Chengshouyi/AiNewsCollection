...
def mock_task_service(monkeypatch, sample_tasks_data, sample_history_data):
    ...
    class MockTaskService:
        ...
        def validate_task_data(self, data, is_update=False):
            # 簡單模擬：檢查必需字段
            if not is_update and 'task_name' not in data:
                 return {'success': False, 'message': '缺少 task_name', 'errors': {'task_name': '必需'}}
            # 修正：crawler_id 應為 int
            crawler_id = data.get('crawler_id')
            if not is_update and (crawler_id is None or not isinstance(crawler_id, int)):
                 return {'success': False, 'message': '缺少或無效的 crawler_id', 'errors': {'crawler_id': '必需且為整數'}}
            # 模擬 ScrapeMode 驗證
            if 'scrape_mode' in data:
                    try:
                     ScrapeMode(data['scrape_mode'])
                    except ValueError:
                     return {'success': False, 'message': f"無效的抓取模式: {data['scrape_mode']}", 'errors': {'scrape_mode': '無效值'}}
            # 模擬成功
        ...
        def create_task(self, data):
            ...
            task_data_full = {
                ...
                'retry_count': 0,
                'scrape_mode': data.get('scrape_mode', ScrapeMode.FULL_SCRAPE.value), # 從驗證過的 data 獲取
        ...
        def get_task_by_id(self, task_id, is_active=None):
            ...
            if is_active is not None and task.is_active != is_active:
                 return {'success': False, 'message': f'任務 {task_id} 的活躍狀態不匹配'}
            # 修正：返回模擬對象
            ...
            for key, value in data.items():
                ...
                if key == 'scrape_mode':
                    ...
                    if isinstance(value, dict):
                        ...
                        task.task_args = value # 允許直接覆蓋（可能不是期望行為）
                else:
        ...
        def advanced_search_tasks(self, **filters):
            ...
            for task in tasks:
                ...
                for k, v in filters.items():
                    ...
                    if isinstance(attr_value, enum.Enum):
                        enum_value = attr_value.value  # 先取得 Enum 的值
                        if isinstance(v, enum.Enum):   # 如果篩選條件也是 Enum
                            if enum_value != v.value:
                                match = False; break
                            elif enum_value != v:         # 如果篩選條件是字串/數字等
                                match = False; break
                    elif attr_value != v:            # 如果屬性值不是 Enum，直接比較
                        match = False; break
                if match:
                    filtered_tasks.append(task)
            # 修正：返回模擬對象列表
            return {'success': True, 'tasks': filtered_tasks, 'total_count': len(filtered_tasks)}

        def get_task_history(self, task_id, limit=None, offset=None, sort_desc=True):
            task = self.tasks.get(task_id)
            if not task:
                 return {'success': False, 'message': '任務不存在', 'histories': [], 'total_count': 0}
            histories = self.task_history.get(task_id, [])
            # 模擬排序
            # 修正：確保 start_time 是 datetime 對象
            ...
            total_count = len(histories_with_dt)
            # 模擬分頁
            if offset is not None and limit is not None:
                 histories_to_return = histories_with_dt[offset:offset + limit]
            elif limit is not None:
                 histories_to_return = histories_with_dt[:limit]
            else:
                histories_to_return = histories_with_dt
            # 修正：返回模擬對象列表
            return {'success': True, 'histories': histories_to_return, 'total_count': total_count}
...
class TestTasksApiRoutes:
    ...
    def test_get_scheduled_tasks(self, client, mock_task_service):
        ...
        # 預期返回 is_scheduled=True, is_active=True, is_auto=True 的任務
        assert isinstance(data, list)
        assert len(data) == 2 # task 1, 2 滿足條件
        assert all(isinstance(task, dict) for task in data)
        assert all(task['is_scheduled'] and task['is_active'] and task['is_auto'] for task in data)
    ...
    def test_create_scheduled_task(self, client, mock_task_service, mock_scheduler_service):
        ...
        assert 'task' in data
        assert data['task']['id'] == 4 # next_id
        assert data['task']['task_name'] == '新排程任務'
        assert data['task']['is_auto'] is True # is_auto=True for scheduled
    ...
    def test_validation_error_on_create(self, client): # 移除 mock_task_service，因為此測試不依賴它
        ...
        data = json.loads(response.data)
        assert data['success'] is False
        # 修正：預期來自 validate_task_data 的消息
        assert '以下必填欄位缺失或值為空/空白: crawler_id' in data['message']
    ...
    def test_validation_error_on_update(self, client, mock_task_service):
        ...
        def failing_validate(data, is_update=False):
            if is_update:
                 ...
                 if 'cron_expression' in data and data['cron_expression'] == '無效的表達式':
                     return {'success': False, 'message': 'Cron 格式無效', 'errors': {'cron_expression': '無效'}}
        ...
        assert 'errors' in data
        assert 'cron_expression' in data['errors']
