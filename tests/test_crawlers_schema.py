"""Tests for the Crawlers schema validation functionality.

This module contains test cases for validating the CrawlersCreateSchema and
CrawlersUpdateSchema classes, ensuring proper data validation and schema behavior.
"""
...
class TestCrawlersUpdateSchema:
    ...
    def test_crawlers_update_schema_with_partial_data(self):
        """測試部分欄位更新"""
        # 只更新 crawler_name
        data1 = {"crawler_name": "updated_crawler", "module_name": "test_module"}
        ...
        # 只更新 is_active
        data3 = {"is_active": False, "module_name": "test_module"}
    ...
    def test_crawlers_update_immutable_fields_validation(self):
        """測試不允許更新不可變欄位的驗證"""
        # 不允許更新 created_at
        data1 = {"crawler_name": "test_crawler", "created_at": datetime.now()}
        with pytest.raises(ValidationError) as exc_info:
            CrawlersUpdateSchema.model_validate(data1)
        assert "不允許更新 created_at 欄位" in str(exc_info.value)

        # 不允許更新 id
        data2 = {"crawler_name": "test_crawler", "id": 1}
        with pytest.raises(ValidationError) as exc_info:
    ...
    def test_crawlers_update_is_active_validation(self):
        """測試更新是否啟用的驗證"""
        # 非布林值
        data_not_bool = {"is_active": "not_a_boolean"}
        with pytest.raises(ValidationError) as exc_info:
