...
class TestCrawlerFactory:
    ...
    def test_initialize(
        self, setup_crawler_factory, sample_crawlers, monkeypatch, mock_article_service
    ):
        ...
        assert (
            CrawlerFactory._crawler_names["TestCrawler"]["class"] == mock_test_crawler
        )
        assert (
            CrawlerFactory._crawler_names["NewsCrawler"]["class"] == mock_news_crawler
        )
    ...
    def test_initialize_exception(self, setup_crawler_factory, mock_article_service):
        ...
        with pytest.raises(Exception) as excinfo:
            CrawlerFactory.initialize(
                mock_service, article_service=mock_article_service
            )
    ...
    def test_get_crawler(
        self, setup_crawler_factory, monkeypatch, mock_article_service
    ):
        """測試獲取爬蟲實例功能"""
        mock_service = setup_crawler_factory
        ...
        mock_crawler_class.assert_called_once_with(
            config_file_name="test_config.json", article_service=mock_article_service
        )
    ...
    def test_get_crawler_not_found(
        self, setup_crawler_factory, monkeypatch, mock_article_service
    ):
        """測試獲取不存在的爬蟲實例"""
        mock_service = setup_crawler_factory
    ...
    def test_list_available_crawlers(
        self, setup_crawler_factory, monkeypatch, mock_article_service
    ):
        """測試列出可用爬蟲功能"""
        mock_service = setup_crawler_factory
