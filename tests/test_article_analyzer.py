...
class TestArticleAnalyzer:
    ...
    def test_is_ai_related_by_content_keywords_above_threshold(self):
        ...
        article = {'content': '本文討論了人工智能、大型語言模型、機器學習和深度學習的最新進展，以及在各行業的應用。'}
        assert ArticleAnalyzer.is_ai_related(article) is True
    ...
    def test_is_ai_related_no_ai_info(self):
        ...
        article = {'category': '生活', 'title': '週末好去處', 'tags': ['旅遊', '美食'], 'content': '今天天氣晴朗，適合出遊。'}
        assert ArticleAnalyzer.is_ai_related(article) is False

    def test_is_ai_related_check_content_false(self):
        """測試 check_content 為 False 時，即使內容包含足夠關鍵字也可能返回 False"""
        article_with_ai_content = {'title': '無關標題', 'content': '機器學習 深度學習 人工智慧'}
        assert ArticleAnalyzer.is_ai_related(article_with_ai_content, check_content=False) is False
        article_with_ai_category = {'category': 'ai', 'content': '無關內容'}
        assert ArticleAnalyzer.is_ai_related(article_with_ai_category, check_content=False) is True
        article_with_ai_title = {'title': 'AI應用', 'content': '無關內容'}
        assert ArticleAnalyzer.is_ai_related(article_with_ai_title, check_content=False) is True
        article_with_ai_tag = {'tags': ['科技', '人工智慧'], 'content': '無關內容'}
        assert ArticleAnalyzer.is_ai_related(article_with_ai_tag, check_content=False) is True

    def test_analyze_articles_statistics_basic(self):
        """測試 analyze_articles_statistics 的基本統計資訊"""
        data = {'title': ['AI文章1', '科技文章', 'AI文章2'],
    ...
    def test_analyze_articles_statistics_ai_keyword_frequency(self):
        ...
        expected_keywords = {keyword: 1 for keyword in ['機器學習', '深度學習', '人工智慧'] if keyword in AI_KEYWORDS}
    ...
    def test_analyze_articles_statistics_missing_columns(self):
        ...
        stats_no_length = ArticleAnalyzer.analyze_articles_statistics(df_no_content_length)
        ...
        stats_no_content = ArticleAnalyzer.analyze_articles_statistics(df_no_content, ai_only=True)
